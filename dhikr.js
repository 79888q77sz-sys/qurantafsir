...
var ADHKAR_CAT_LABELS = {
  ...
  house_enter:    'چوونا ماڵ',
  house_exit:     'دەرچوون ژ ماڵ',
  bathroom_enter: 'چوونا دەستئاڤێ',
  bathroom_exit:  'دەرچوونا دەستشوینکێ',
  masjid_enter:   'چوونا مزگەفتێ',
  masjid_exit:    'دەرچوونا ژ مزگەفتێ',
  wudu:           'دەستنڤێژ شویشتن',
  opening_prayer: 'دوعایا دەستپێکرنا نڤێژێ',
  ruku:           'زکرێن ڕکووعێ',
  rising_ruku:    'ڕاستبوونا ڕکووعێ',
  sujood:         'زکرێن سوجدێ',
  between_sujood: 'ناڤبەرا هەردوو سوجدەیان',
  tashahhud:      'تەشەهود',
  eating_before:  'بەری خوارنێ',
  eating_after:   'پشتی خوارنێ',
  breaking_fast:  'دەمێ فتارێ',
  fasting:        'ئنیەتا ڕۆژیگرتنێ',
  travel:         'گەشت',
  travel_return:  'ڤەگەڕیان بۆ ماڵ',
  vehicle:        'سواربوونا هویێن ڤەگوهاستنێ',
  marketplace:    'چوونا بازاری',
  charity:        'دەمێ سەدەقێ',
  distress:       'پەریشانی',
  debt:           'دەینداری',
  fear:           'دەمێ ترسێ',
  forgiveness:    'داخوازا غەفراندنا گونەهان',
  protection:     'پاراستن',
  rain:           'باران',
  thunder:        'دەمێ بریسییان',
  wind:           'هاتنا هەوای',
  after_prayer:   'پشتی نڤێژێ',
  adhan:          'پشتی بانگی',
  salawat:        'صەڵەوات',
  qunut:          'دوعایا قونووتێ',
  lailat_qadr:    'شەڤا قەدرێ',
  dhul_hijjah:    'دەهکێن ذولحیجەیێ',
  talbiyah:       'تەلبیەی حەج',
  safa_marwa:     'سەفا و مەروا',
  zamzam:         'ئاڤا زمزم',
  entering_makkah:'چوونا مەکەهێ',
  arafat:         'دوعایا عەرەفاتێ',
  grave:          'سەرەدانا گۆرستانێ',
  eclipse:        'دەمێ کوسوفێ',
  illness:        'نەخۆشی',
  pain:           'دەمێ نەخۆشبوونێ',
  ruqyah:         'ڕووقیا شەرعی',
  visiting_sick:  'سەرەدانا نەخۆشی',
  condolence:     'دلتەنگی',
  newborn:        'زێدەبوونا زارۆکان',
  anger:          '٣ دەم',
  sneeze:         'دەمێ بێهنشینێ',
  new_clothes:    'جلێن نوێ',
  dressing:       'دەمێ لبەرخۆکرنا جلوبەرگان',
  mirror:         'دیتنا خۆدیکێ',
  new_moon:       'دیتنا هەیڤا نوی',
  nightmare:      'خەوا خراب',
  istikhara:      'دوعایا ئیستیخارێ',
  wedding:        'پیرۆزباهییا هەڤژینییێ',
  kaffarah:       'ڕابوون ژ دیوانەکێ',
...
/* ── Chunked list building ──
   Builds the first `firstN` items synchronously (enough to fill the viewport)
   and streams the rest into `list` in rAF batches of `perFrame`, so opening a
   section never blocks the tap frame behind a full-list DOM build. A newer
   build on the same list element (e.g. a search keystroke) cancels any
   batches still pending from the previous build. */
function _chunkAppend(list, items, makeEl, firstN, perFrame, onDone) {
  var gen = (list._chunkGen = (list._chunkGen || 0) + 1);
  var i = 0, n = items.length;
  var end0 = Math.min(n, firstN);
  for (; i < end0; i++) list.appendChild(makeEl(items[i], i));
  if (i >= n) { if (onDone) onDone(); return; }
  function step() {
    if (list._chunkGen !== gen) return; /* superseded by a rebuild */
    var frag = document.createDocumentFragment();
    var end = Math.min(n, i + perFrame);
...
function _triggerBgRefresh() {
  ...
  _fetchDbData(function() {
    ...
    if (!ui) return;
    ui._homeEl = null;
    /* A silent refresh must never yank the user's scroll to the top — hand the
       position to each view's own restore slot where one exists, otherwise
       re-apply it directly after the redraw. */
    var _bgp = document.getElementById('gencineContent');
    var _bgPos = _bgp ? _bgp.scrollTop : 0;
    if (ui._view === 'home') {
      if (ui._homeScrollPos == null) ui._homeScrollPos = _bgPos;
      ui._draw();
    }
    else if (ui._view === 'hadith' && ui._hadithDetailIdx === null) {
      if (ui._hadithListScroll == null) ui._hadithListScroll = _bgPos;
      ui._draw();
    }
    else if (ui._view === 'books') {
      if (ui._booksScrollPos == null) ui._booksScrollPos = _bgPos;
      ui._draw();
    }
    else if (ui._view === 'tasbih') ui._draw();
    else if (ui._view === 'adhkar' && ui._adhkarView === 'grid') {
      if (ui._adhkarGridScrollPos == null) ui._adhkarGridScrollPos = _bgPos;
      ui._draw();
    }
    else if (ui._view === 'adhkar' || ui._view === 'dua' || ui._view === 'asma') {
      /* chunk-rendered lists — the saved position may lie beyond the first
         chunk, so retry once after the remaining chunks land */
      ui._draw();
      if (_bgp && _bgPos > 0) {
        requestAnimationFrame(function(){ _bgp.scrollTop = _bgPos; });
        setTimeout(function(){ if (_bgp.scrollTop < _bgPos - 8) _bgp.scrollTop = _bgPos; }, 400);
      }
    }
  });
}
...
window.GencineUI = {
  ...
  render: function(){
    var self = this;
    this._loadState();
    /* The prewarm IIFE sets _dbLoaded from cache without ever reaching
       _initDbData, so warm sessions would otherwise never register the
       network-recovery/foreground listeners. Idempotent. */
    _registerConnListeners();
  ...
  section: function(name){
    ...
    this._view = name;
    if (name === 'hadith') { this._hadithDetailIdx = null; this._hadithSearch = ''; this._hadithListScroll = null; }
  ...
  _updateHeader: function(){
    ...
    // Sort button lives in the header next to search — hadith list only
    var gencSortBtn = document.getElementById('gencSortBtn');
    if(gencSortBtn) gencSortBtn.style.display = (this._view === 'hadith' && this._hadithDetailIdx === null) ? 'flex' : 'none';
  },
  ...
  _renderHome: function(container){
    ...
    if (_soonest) {
      setTimeout(function() {
        ...
        if (self._view === 'home') {
          var _pg = document.getElementById('gencineContent');
          if (_pg && self._homeScrollPos == null) self._homeScrollPos = _pg.scrollTop;
          self._draw();
        }
  ...
  _renderAdhkarCatList: function(container){
    ...
    _chunkAppend(list, items, makeAdhkarCard, 6, 8, function(){ list.appendChild(comp); });
  ...
  _renderDua: function(container){
    ...
    function buildDuaList(q) {
      ...
      _chunkAppend(list, items, makeDuaCard, 4, 6);
    }
  ...
  _renderTasbih: function(container){
    ...
    /* tap handled by the wrap-level delegate below — whole view is the tap surface */
    ...
    // warm twisted-cord tones (was bare metal) — reads like a real misbaha thread
    [['0%','#8a6f4f'],['50%','#cdb693'],['100%','#8a6f4f']].forEach(function(st){
    ...
    /* count pop — one reusable element rising from the center bead per count.
       Lives on beadWrap (NOT the stage): the stage clips overflow, which cut
       the number off as it rose past the zikr-selector card above. */
    var countPop = document.createElement('div');
    countPop.className = 'tasbih-count-pop';
    countPop.id = 'tasbihCountPop';
    var _popNum = document.createElement('div');
    _popNum.className = 'tasbih-pop-num';
    countPop.appendChild(_popNum);
    var _popZikr = document.createElement('div');
    _popZikr.className = 'tasbih-pop-zikr';
    countPop.appendChild(_popZikr);
    beadWrap.appendChild(countPop);
    ...
    /* Whole-view tap surface — the bead strip alone was too small a target.
       Any tap that isn't on a control (dhikr selector, picker sheet, target
       presets, reset/voice row) counts; bead-stage and count-area taps reach
       here by bubbling, so they must NOT have their own onclick (double count). */
    wrap.onclick = function(e){
      if (e.target.closest('.tasbih-sel-card,.tasbih-picker-sheet,.tasbih-target-row,.tasbih-action-row')) return;
      self._tasbihTap();
    };
  ...
  _renderHadith: function(container){
    ...
    searchInner.appendChild(searchIco); searchInner.appendChild(searchInput); searchInner.appendChild(searchClear);
    ...
    function buildList(q, firstN) {
      ...
      _chunkAppend(list, scored, makeHadithRow, firstN || 12, 24);
    }

    /* Restore the saved list scroll when returning from a detail view. The list
       is chunk-rendered, so sync-render enough rows to cover the saved offset
       first (56px is a safe row-height floor), then re-apply after layout. */
    var _hls = this._hadithListScroll != null ? this._hadithListScroll : null;
    this._hadithListScroll = null;
    if (_hls > 0) {
      buildList(this._hadithSearch, Math.min(hadiths.length, Math.ceil(_hls / 56) + 10));
      var _pnl = document.getElementById('gencineContent');
      if (_pnl) requestAnimationFrame(function(){ _pnl.scrollTop = _hls; });
    } else {
      buildList(this._hadithSearch);
    }

    /* Sort toggle — lives in the header beside the search icon */
    var _gsrtHadith = document.getElementById('gencSortBtn');
    if (_gsrtHadith) {
      _gsrtHadith.classList.toggle('on', !!this._hadithReverse);
      _gsrtHadith.onclick = function(){
        self._hadithReverse = !self._hadithReverse;
        _gsrtHadith.classList.toggle('on', self._hadithReverse);
  ...
  _tasbihTap: function(){
    ...
    this._popCount();   // count + zikr rise from the center bead (tap AND voice)
    this._saveState();
    this._updateRing();
  },

  /* Floating count pop: reuses one element; removing+re-adding the class
     restarts the CSS animation so rapid taps retrigger cleanly. */
  _popCount: function(){
    var pop=$('tasbihCountPop');
    if(!pop)return;
    var num=pop.querySelector('.tasbih-pop-num');
    var zk=pop.querySelector('.tasbih-pop-zikr');
    if(num)num.textContent=this._tasbihCount;
    if(zk){
      var d=(_getTasbih()[this._tasbihDhikrIdx]||{});
      zk.textContent=d.ar||'';
    }
  ...
  _tasbihReset: function(){
    var hadCount = this._tasbihCount > 0;
    this._tasbihCount = 0;
    this._saveState();
    if(hadCount && $('tasbihBeadRail') && this._beadOrder && this._beadOrder.length){
      // Rewind animation: the chain rolls backward into 0 with the same spring
      // used for counting. Bead layout is periodic per slot (identical pearls),
      // so re-basing a big count down to ~12 slots is visually seamless and
      // keeps the rewind readable (~0.6s) instead of a 100+-slot blur.
      if(this._beadRotC > 12){
        this._beadRotC = 12 + (this._beadRotC % 1);
        this._beadFrame(this._beadRotC);
      }
      this._beadRotT = 0;
      this._beadSpring();
      window.H && window.H.medium();
    }
    this._updateRing();
  },
  ...
  /* position every bead for slide offset `rot` (float, in slots).
     The perspective projection is computed HERE in JS — the exact same
     equation _drawBeadThread uses for the wire — never delegated to CSS
     perspective. The projected point is the bead's mathematical center:
     translate3d moves the box so its center lands on (sx,sy), then scale()
     grows/shrinks it about that same center (transform-origin:center), so
     the bead stays threaded on the wire at every size. */
  _beadFrame: function(rot){
    ...
    var N = this._BEAD_N, SP = this._BEAD_SP, half = this._BEAD_SIZE / 2, P = this._BEAD_PERSP;
    for(var i = 0; i < N; i++){
      var s = (i - rot) % N; if(s < 0) s += N;   // wrapped slot 0..N — recycle happens off-screen
      var pt = this._beadPoint(g, (s - N / 2) * SP);
      var f = P / (P - pt.z);                     // projection factor — identical to the wire's
      var sx = g.cx + (pt.x - g.cx) * f;
      var sy = g.oy + (pt.y - g.oy) * f;
      var el = order[i];
      el.style.transform = 'translate3d(' + (sx - half).toFixed(2) + 'px,' + (sy - half).toFixed(2) + 'px,0) scale(' + f.toFixed(4) + ')';
      el.style.opacity = (1 - .4 * pt.t).toFixed(3);          // fades to 0.6 at the edges
      el.style.zIndex = 100 - Math.round(pt.t * 50);          // center bead paints on top
      // Authentic fasil (divider) bead every 33 counts — like a real misbaha.
      // rot+(s−N/2) is CONSTANT for a physical bead between wraps, so the
      // amber marker never hops between beads mid-animation.
      var li = Math.round(rot + (s - N / 2));
      el.classList.toggle('tb-bead-marker', li > 0 && li % 33 === 0);
    }
  },
  ...
  _beadSpring: function(){
    var self = this;
    // Low-power path: critical-tier devices and reduced-motion users get an
    // instant snap instead of ~30 rAF frames writing 28 transforms each.
    try{
      if((window.TKPerf&&window.TKPerf.level==='critical')||
         (window.matchMedia&&matchMedia('(prefers-reduced-motion:reduce)').matches)){
        if(self._beadRaf){cancelAnimationFrame(self._beadRaf);self._beadRaf=null;}
        self._beadRotC=self._beadRotT;self._beadVel=0;
  ...
  _renderBooks: function(container){
    ...
    function renderGrid() {
      ...
      var _bookCardIdx = 0;
      function makeGridItem(book){
        if (book._isSeries) { return _buildSeriesCard(book); }
      ...
      /* Restoring scroll (back from reader) needs the grid's full height on
         this frame — render everything synchronously in that case. */
      var _syncAll = self._booksScrollPos != null;
      _chunkAppend(grid, _renderItems, makeGridItem, _syncAll ? _renderItems.length : 6, 8);
    }
