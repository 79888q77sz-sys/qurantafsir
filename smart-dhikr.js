/**
 * Smart Daily Companion  v86
 * Variable number of slides — seasonal items each get own slide, never displace card 1:
 *
 * v86: fully-automated model (SmartContext v2) — no signal-gated general
 *      duas; distress/gratitude/protection/house/dressing/nightmare join an
 *      idle-time DISCOVERY rotation (additive score, per-hour+per-session
 *      randomization, shown-decay anti-repeat via noteShown). Completed
 *      top-scorer now hands over to the next fresh candidate.
 * v85: SmartContext integration (smart-context-engine.js) — dynamic scoring,
 *      lateNight tahajjud, boundary-scheduled watcher replaces the 20 s poll.
...
(function(window) {
  ...
  function _catHasData(catKey) {
    try {
      ...
      var list = (raw && Array.isArray(raw.data)) ? raw.data : (Array.isArray(raw) ? raw : null);
      if (!list) return true; /* cache not populated yet — show optimistically */
      return list.some(function(a) { return a.category_key === catKey && a.active !== false; });
    } catch(e) { return true; }
  }
  ...
  /* Legacy flat score — kept as the fallback path when smart-context-engine.js
     is unavailable (load failure). SmartContext.score supersedes it. */
  ...
  /* ─────────────────────────────────────────────
     SMARTCONTEXT WIRING (smart-context-engine.js)
     Scripts load in parallel, so wiring is lazy — first use configures
     the engine with THIS file's battle-tested providers (hijri, weather,
     prayers stay implemented in one place) and registers the boundary
     watcher. If the engine never loads, every caller falls back to the
     v84 behavior: legacy scoring + 20 s interval watcher.
  ───────────────────────────────────────────── */
  var _scReady = false;

  function _sc() {
    var SC = window.SmartContext;
    if (!SC) { _startLegacyWatcher(); return null; }
    if (!_scReady) {
      _scReady = true;
      /* engine arrived after a legacy interval started — retire the poll */
      if (_legacyWatchTid) { clearInterval(_legacyWatchTid); _legacyWatchTid = null; }
      SC.configure({
        getPrayers:       _getPrayerTimings,
        getHijri:         function() { return _toHijri(_baghdadDate()); },
        getDhulHijjahDay: _getDhulHijjahDay,
        getWeather:       _getWeatherCondition
      });
      SC.onBoundary(_contextTick);
    }
    return SC;
  }
  ...
  function _getZikrItem() {
    ...
    return _pickZikrAt(null, false);
  }

  /* Core card-1 selection at an arbitrary instant. atMs=null → now.
     silent=true (widget timeline simulation): no noteShown feed, no debug
     logs — the rotation must only decay for what the USER actually saw. */
  function _pickZikrAt(atMs, silent) {
    ...
    var SC   = _sc();
    var ctx  = SC ? SC.snapshot(atMs || undefined) : null;
    var pool = ctx ? TIME_ITEMS.concat(SC.CONTEXT_ITEMS) : TIME_ITEMS;

    var active = pool
      .filter(function(item) {
        if (!_catHasData(item.categoryKey)) return false;
        /* SmartContext-only item kinds (phase / discovery / requiresSignal):
           the engine gates them — without it they simply never activate */
        if (item.phase || item.discovery || item.requiresSignal)
          return ctx ? SC.score(item, ctx, null).eligible : false;
        return _isTimeActive(item, nowMin, dow, prayers, maghribMin, fajrMin);
      })
      .map(function(item) {
        /* Dynamic score: base × timeFit trapezoid × calendar × signal boosts
           − fatigue. Legacy flat score only if the engine is missing. */
        var s = ctx ? SC.score(item, ctx, state).score : _scoreItem(item, state);
    ...
    /* Any completed winner yields — score system already penalises by -60,
       but if the top-scorer is still completed, hand over to the next fresh
       candidate (the discovery pool usually has one) before resorting to
       the static seeded fallback. */
    var winnerExhausted = winner && state.completed.indexOf(winner.id) >= 0;
    if (winnerExhausted) {
      for (var wi = 1; wi < active.length; wi++) {
        if (state.completed.indexOf(active[wi].item.id) < 0) {
          winner = active[wi].item;
          winnerExhausted = false;
          break;
        }
      }
    }
    ...
    if (!silent) try {
      ...
      if (active.length) {
        ...
        active.forEach(function(c) {
          var mark = (!winnerExhausted && c.item === winner) ? '✓' : ' ';
    ...
    var chosen = (!active.length || winnerExhausted) ? fallback : winner;
    /* feed the discovery rotation — what surfaced now decays next hour */
    if (!silent && SC && SC.noteShown && chosen) SC.noteShown(chosen.id);
    return chosen;
  }

  /* ─────────────────────────────────────────────
     WIDGET TIMELINE PUSH — precomputed 24 h schedule for the native
     Smart-Card widgets (iOS WidgetKit + Android SmartCardWidgetProvider).
     The engine is never ported: this walks boundary→boundary re-running
     the EXACT card-1 selection above and ships the winners as JSON via
     the SharedPrefs bridge (iOS App Group / Android SharedPreferences).
     The `widget` key prefix auto-triggers native refresh on both platforms.
  ───────────────────────────────────────────── */
  var _ICON_KEYS = {
    'fa-sun': 'sun', 'fa-cloud-sun': 'sun', 'fa-mosque': 'mosque',
    'fa-moon': 'moon', 'fa-cloud-moon': 'moon', 'fa-bed': 'moon',
    'fa-star-and-crescent': 'crescent', 'fa-hands-praying': 'hands',
    'fa-hand-holding-heart': 'heart', 'fa-heart-pulse': 'heart',
    'fa-shield-halved': 'shield', 'fa-star': 'star',
    'fa-door-open': 'door', 'fa-house-user': 'house', 'fa-shirt': 'shirt',
    'fa-cloud-rain': 'rain', 'fa-cloud-bolt': 'rain', 'fa-wind': 'rain',
    'fa-utensils': 'food', 'fa-kaaba': 'kaaba'
  };
  function _iconKey(faClass) {
    if (!faClass) return 'sparkles';
    var parts = String(faClass).split(/\s+/);
    for (var i = 0; i < parts.length; i++) {
      if (_ICON_KEYS[parts[i]]) return _ICON_KEYS[parts[i]];
    }
    return 'sparkles';
  }

  /* Deterministic per-day shuffle of 0..n-1 — same order all day (widget
     content stays coherent across re-pushes), reshuffles at Baghdad midnight.
     Seeded Fisher–Yates over a tiny LCG; salt mixes the category in. */
  function _dayShuffledOrder(n, saltStr) {
    var seed = _daySeed() >>> 0;
    for (var c = 0; c < saltStr.length; c++) seed = (seed * 31 + saltStr.charCodeAt(c)) >>> 0;
  ...
  function _timelineEntryFor(item, fromMs) {
    ...
    var title      = T(item.labelKey, item.labelFallback);
    var arabic     = featured && featured.ar ? featured.ar.replace(/\s+/g, ' ').trim() : (item.fallbackAr || '');
    if (arabic.length > 120) arabic = arabic.slice(0, 120) + '…';
    var rep = featured ? (featured.repeat || 1) : (item.fallbackRepeat || 1);
    var src = (featured ? featured.source : item.fallbackSource) || '';
    var subParts = [title];
    if (rep > 1) subParts.push('× ' + rep);
    if (src)     subParts.push(src);

    /* Intra-category rotation pool (widgets only — the app slider still shows
       the single daily featured dhikr). Day-stable shuffled selection of up to
       8 real adhkar from the winning category; featured leads so a widget
       glanced right after the app matches it. Natives rotate through this
       pool in 15-min buckets (Android alarm-on-wake / iOS sub-entries). */
    var pool = [];
    if (adhkarList.length) {
      var order = _dayShuffledOrder(adhkarList.length, item.categoryKey);
      for (var pi = 0; pi < order.length && pool.length < 8; pi++) {
        var a = adhkarList[order[pi]];
        if (!a || !a.ar || a === featured) continue;
        var ar = String(a.ar).replace(/\s+/g, ' ').trim();
        if (!ar) continue;
        if (ar.length > 120) ar = ar.slice(0, 120) + '…';
        pool.push({ ar: ar, repeat: a.repeat || 1, source: a.source || '' });
      }
    }
    /* featured first — pool[0] is what the plain fields also carry */
    pool.unshift({ ar: arabic, repeat: rep, source: src });

    return {
      fromMs: fromMs, id: item.id,
      tag:    item.timeTag || T(item.subtitleKey, item.subtitleFallback) || '',
      title:  title,
      count:  adhkarList.length,
      sub:    subParts.join('  ·  '),
      arabic: arabic, repeat: rep, source: src,
      icon:   _iconKey(item.icon),
      /* exact FA class name (e.g. "fa-star-and-crescent") — Android renders
         this glyph from the bundled fa-solid font so the widget icon matches
         the in-app card exactly; `icon` above stays the coarse key iOS maps
         to SF Symbols. */
      iconFa: (String(item.icon || '').split(/\s+/).filter(function (p) {
  ...
  function pushSmartWidgetTimeline() {
    try {
      var sp = window.Capacitor && Capacitor.Plugins && Capacitor.Plugins.SharedPrefs;
      var SC = window.SmartContext;
      if (!sp || !SC || !SC.nextBoundaryDeltaMs) return;
      var entries = [];
      // 48 h horizon (zero-empty-state): widgets stay time-correct through a
      // full weekend without an app open; natives clamp to the last entry
      // beyond that, and their static fallback floors cover the rest.
      var t = Date.now(), horizon = t + 48 * 3600000, guard = 0, lastId = null;
      while (t < horizon && guard++ < 96 && entries.length < 80) {
        var item = _pickZikrAt(t, true);
        if (item && item.id !== lastId) {
          entries.push(_timelineEntryFor(item, t));
          lastId = item.id;
        }
        t += SC.nextBoundaryDeltaMs(t);
      }
      if (!entries.length) return;
      sp.set({ key: 'widgetSmartTimeline',
               value: JSON.stringify({ v: 1, generatedAt: Date.now(),
                                       tz: 'Asia/Baghdad', entries: entries }) });
      console.log('[SmartWidget] timeline pushed — ' + entries.length + ' entries');
    } catch (e) { console.warn('[SmartWidget] push failed: ' + (e && e.message)); }
  }
  ...
  function _initSlider(wrapper, track, dotsEl, count) {
    ...
    function _updateCardScales(xPos) {
      ...
      for (var i = 0; i < _slides.length; i++) {
        ...
        if (d >= 2) {
          // Resting state is CONSTANT out here (0.92/0.78) regardless of
          // xPos, yet this runs on every rAF frame of a swipe/auto-advance
          // (up to 60x/sec) — writing transform+opacity to every offscreen
          // clone every frame for values that never change is pure wasted
          // paint work, and on a weaker phone that's exactly the kind of
          // frame drop that reads as a slight stutter/"shake" during the
          // slide transition. Touch the DOM once per entry into this zone
          // instead of every frame; behavior is identical, just not redone
          // 60 times a second for nothing.
          if (!_slides[i]._sdFar) {
            _slides[i]._sdFar = true;
            _slides[i].style.transform = 'scale(0.920) translateZ(0)';
            _slides[i].style.opacity   = '0.780';
          }
          continue;
        }
        _slides[i]._sdFar = false;
        var sc = d < 1 ? (1 - d * 0.05) : (0.95 - (d - 1) * 0.03);
        var op = d < 1 ? (1 - d * 0.12) : (0.88 - (d - 1) * 0.10);
  ...
  function render(gencineUI) {
    ...
    /* Native Smart-Card widgets: push the fresh 24 h schedule off the render
       path (card-1 was just chosen + noteShown, so decay state is current).
       Debounced — render can run several times during one boot. */
    clearTimeout(window._swtPushTid);
    window._swtPushTid = setTimeout(pushSmartWidgetTimeline, 1200);
  ...
  /* ─────────────────────────────────────────────
     ...
     Driven by SmartContext.onBoundary — ONE timeout armed to the next
     moment the answer can change (prayer edge, last-third start, Baghdad
     midnight, signal expiry), re-armed on visibilitychange/resume. No
     polling. The 20 s interval survives only as the fallback when the
     engine failed to load (registered via _startLegacyWatcher in _sc).
  ───────────────────────────────────────────── */
  var _prevSeasonalKey = null;
  var _prevZikrId      = null;
  var _prevDaySeed     = null;
  var _legacyWatchTid  = null;

  function _contextTick() {
    try {
      ...
      /* Keep the native Smart-Card widgets in lockstep: every boundary tick
         (and every app resume, which re-fires it) re-pushes the 24 h schedule
         with fresh decay/fatigue state. Debounced natively on both platforms. */
      pushSmartWidgetTimeline();
    } catch(e) {}
  }

  function _startLegacyWatcher() {
    if (_legacyWatchTid || _scReady) return;
    _legacyWatchTid = setInterval(_contextTick, 20 * 1000);
  }
